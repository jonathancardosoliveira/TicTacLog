...
% modo_jogo/1 (dinamico): armazena o modo escolhido pelo usuario em tempo de execucao.
% Valores possiveis: pessoa (dois jogadores humanos) ou ia (humano vs computador).
:- dynamic modo_jogo/1.
...
% Se o modo for ia e for o jogador 2, exibe "IA (O)" em vez de "Jogador 2 (O)".
nome_jogador(1, 'Jogador 1 (X)').
nome_jogador(2, Nome) :-
    modo_jogo(ia) -> Nome = 'IA (O)' ; Nome = 'Jogador 2 (O)'.

% proximo_jogador/2: alterna o turno entre Jogador 1 (X) e Jogador 2 (O ou IA).
proximo_jogador(1, 2).
proximo_jogador(2, 1).
...
% Testa as 8 combinacoes possiveis: 3 horizontais, 3 verticais e 2 diagonais.
...
% Caso base (Pos=1): troca a cabeca da lista. Recursivo: avanca na cauda.
...
% Gera todas as jogadas possiveis em casas vazias, avalia cada uma com valor_minimax/5
% e seleciona a de maior pontuacao (sort/4 ordena decrescente e pega a primeira).
...
% valor_minimax/5: calcula pontuacao de um estado do tabuleiro para a IA.
% Parametros: Tab, JogIA (jogador da IA), Prof (profundidade), Maximizando (bool), Val (saida).
% Pontuacao: +10-Prof se IA vence, Prof-10 se oponente vence, 0 se empate.
% Quanto menor Prof, melhor a vitoria (prioriza vencer mais cedo).
...
% No da IA (maximizador): simula todas as jogadas possiveis da IA e retorna o maior valor
...
% max_lista/2 e min_lista/2: auxiliares para obter maior e menor valor de uma lista.
% Implementacao recursiva manual (SWI-Prolog nao exige, mas evita dependencias).
...
% iniciar/0: ponto de entrada — exibe menu e le a opcao de modo de jogo.
iniciar :-
    nl,
    write('=================================================='), nl,
...
% definir_modo/1: configura modo_jogo/1 conforme opcao digitada e inicia a partida.
% Opcao invalida reinicia o menu. retractall/1 limpa modo anterior antes de assert/1.
definir_modo(1) :-
...
% iniciar_partida/0: carrega tabuleiro inicial e comeca o jogo com Jogador 1 (X).
iniciar_partida :-
    nl,
...
% rodar/3: loop principal do jogo — exibe tabuleiro e obtem a proxima jogada.
% Se for vez do Jogador 2 no modo ia, a IA joga automaticamente via melhor_jogada/4.
% Caso contrario, le a jogada do teclado com read/1.
...
% processar_jogada/4: trata entrada do jogador (ou da IA) em quatro situacoes:
%   1) Saida voluntaria (-1, -1)
%   2) Jogada valida — verifica vitoria, empate ou continua o jogo
%   3) Jogada invalida — posicao inexistente ou casa ocupada
%   4) Formato de entrada incorreto

% Caso 1: encerramento voluntario do jogo
...
% Caso 2a: jogada valida — atualiza tabuleiro e verifica fim de jogo
processar_jogada((Linha, Coluna), Tab, Jogador, NovoTab) :-
...
% Caso 2b: coordenadas validas, mas jogada impossivel (casa ocupada ou fora do tabuleiro)
processar_jogada((Linha, Coluna), Tab, Jogador, NovoTab) :-
    integer(Linha), integer(Coluna),
    !,
    write('--------------------------------------------------'), nl,
...
% Caso 3: entrada com formato invalido
